...
if [ $(cat /proc/sys/kernel/hostname) == 'master01' ]; then
  set -e
  [ -d /usr/local/src/kubeasy ] && rm -rf /usr/local/src/kubeasy
  ...
  sudo kubectl apply -f https://raw.githubusercontent.com/rancher/local-path-provisioner/v0.0.28/deploy/local-path-storage.yaml
  sudo kubectl patch storageclass local-path -p '{"metadata": {"annotations":{"storageclass.kubernetes.io/is-default-class":"true"}}}'

  sudo kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.14.4/config/manifests/metallb-native.yaml
  sudo kubectl -n metallb-system rollout status deployments/controller
...
helm upgrade -i kok ./kok -n kok --create-namespace \
--set webhookUrl=http://$(kubectl get svc -n kok kok -o jsonpath='{.status.loadBalancer.ingress[0].ip}'):8080
fi
